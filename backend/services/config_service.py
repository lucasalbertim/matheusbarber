...
class ConfigService:
    ...
    def get_all_configs(db: Session) -> Dict[str, Any]:
        ...
        for config in configs:
            # Tentar fazer parse de JSON para configurações complexas
            try:
    ...
    def get_attendance_mode_config(db: Session) -> Dict[str, Any]:
        """Obter configurações específicas do modo de atendimento"""
        configs = {
    ...
    def update_attendance_mode_config(db: Session, config_data: Dict[str, Any]) -> Dict[str, Any]:
        """Atualizar configurações do modo de atendimento"""
        ...
        if 'appointment_scheduled_days' in config_data:
            # Converter lista de dias para string separada por vírgulas
